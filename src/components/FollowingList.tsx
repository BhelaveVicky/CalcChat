...
import { X, Search, UserCheck, ChevronRight } from 'lucide-react';
      ...
      <div className={`w-full max-w-md rounded-3xl p-5 shadow-2xl border max-h-[85vh] flex flex-col ${
        ...
        <div className="flex items-center justify-between pb-3 border-b border-gray-200 dark:border-[#202c33] shrink-0">
          <div className="flex items-center gap-2">
            <UserCheck className="w-5 h-5 text-[#00a8ff]" />
            <h3 className="font-bold text-lg">{title} ({following.length})</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[200px]">
          {filteredFollowing.length > 0 ? (
            filteredFollowing.map((userItem) => {
              const avatar = userItem.photoURL || userItem.avatar;
              const username = userItem.username ? `@${userItem.username.replace(/^@/, '')}` : '@user';

              return (
                <div
                  key={userItem.uid}
                  onClick={() => {
                    if (onSelectUser) onSelectUser(userItem.uid);
                    onClose();
                  }}
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center text-gray-400">
              <UserCheck className="w-10 h-10 text-gray-500 mb-2 opacity-50" />
