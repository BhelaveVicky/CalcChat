...
export const PinnedMessageBanner: React.FC<PinnedMessageBannerProps> = ({
  ...
  return (
    <div className={`px-3 py-2 flex items-center justify-between border-b z-20 text-xs shadow-sm transition-all animate-slide-down ${
      isDark 
        ? 'bg-[#182229]/95 border-pink-500/30 text-[#e9edef]' 
      ...
      >
        <div className="p-1.5 rounded-full bg-pink-500/20 text-pink-500 shrink-0 group-hover:scale-110 transition-transform">
          <Pin className="w-3.5 h-3.5 fill-pink-500" />
        </div>

        <div className="min-w-0 flex-1 border-l-2 border-pink-500 pl-2">
          <div className="font-bold text-pink-500 text-[11px] flex items-center gap-1 uppercase tracking-wider">
            Pinned Message
          </div>
      ...
      <button
        ...
        className="p-1 rounded-full text-gray-400 hover:text-pink-500 hover:bg-pink-500/10 transition-colors ml-2 shrink-0 cursor-pointer"
        title="Unpin message"
      >
        <X className="w-4 h-4" />
      </button>
