...
import { Moon, Sun, History, Globe, ChevronRight, X, Eye, EyeOff, Trash2, Download, Calendar, Clock, Lock, Shield, LogOut, AlertTriangle, Mail, RefreshCw, KeyRound, Check, Loader2, Volume2, VolumeX, Smartphone, HelpCircle, Sparkles, Database, Calculator as CalcIcon, Key, Copy } from 'lucide-react';
        ...
        <div className={`rounded-2xl shadow-sm border overflow-hidden transition-colors ${
          ...
          <div className={`px-5 py-4 flex items-center justify-between transition-colors ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-800'}`}>Button Click Sound</span>
                <p className={`text-xs mt-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Play audio tone on keypad tap</p>
              </div>
            </div>
            <button
              onClick={() => updateSettings('soundEnabled', !settings.soundEnabled)}
              className={`relative w-14 h-8 rounded-full transition-all duration-300 ${
                settings.soundEnabled ? 'bg-gradient-to-r from-amber-500 to-orange-600' : isDark ? 'bg-gray-700' : 'bg-gray-300'
              }`}
            >
              <div
                className={`absolute top-1 w-6 h-6 rounded-full bg-white shadow-md transition-all duration-300 ${
          ...
          {/* Vibration Feedback */}
          <div className={`px-5 py-4 flex items-center justify-between transition-colors border-t ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-800'}`}>Haptic Vibration</span>
                <p className={`text-xs mt-0.5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Vibrate on button press</p>
              </div>
            </div>
            <button
              onClick={() => updateSettings('vibrationEnabled', !settings.vibrationEnabled)}
              className={`relative w-14 h-8 rounded-full transition-all duration-300 ${
                settings.vibrationEnabled ? 'bg-gradient-to-r from-purple-500 to-pink-600' : isDark ? 'bg-gray-700' : 'bg-gray-300'
              }`}
            >
              <div
                className={`absolute top-1 w-6 h-6 rounded-full bg-white shadow-md transition-all duration-300 ${
