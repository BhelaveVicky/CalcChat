...
export const SUPER_ADMIN_EMAIL = 'bhelavevicky66@gmail.com';
export const ADMIN_EMAIL = 'vickybhelave25@navgurukul.com';

export const WELCOME_MESSAGE_TEXT = `👋 **Welcome to CalcChat App!**

Hello and welcome! 🎉

I'm **Vicky Bhelave**, the developer of **CalcChat App**, and I'm excited to have you here.

✅ Your account has been created successfully!

You can now chat with your friends, make voice and video calls, share photos, videos, and files, and enjoy all the features that CalcChat has to offer.

If you need any help or encounter any issues while using the app, feel free to send us a message in this chat. I'll do my best to assist you as quickly as possible.

💡 **Your feedback is very important to us.**
If you think a feature is missing or have an idea that could make CalcChat even better, please let us know. We carefully review every suggestion and will do our best to add useful features as soon as possible.

Thank you for your support and for being a part of the CalcChat community. We hope you enjoy using **CalcChat App**.

Have a wonderful experience! ❤️

**— Vicky Bhelave**
**Developer – CalcChat App**`;
...
interface VaultContextType {
  ...
  activeMentionNotification: AppNotification | null;
  dismissMentionNotification: () => void;
...
export const VaultProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeMentionNotification, setActiveMentionNotification] = useState<AppNotification | null>(null);
  ...
  useEffect(() => {
    ...
    const sendWelcomeMessageIfNeeded = async (targetUser: FirebaseUser | { uid: string; email?: string; displayName?: string }) => {
      ...
      try {
        ...
        const saSnap = await getDocs(saQuery).catch(() => null);
        if (saSnap && !saSnap.empty) {
          superAdminUid = saSnap.docs[0].id;
        } else {
          await setDoc(doc(db, 'users', 'super_admin_vicky'), {
            uid: 'super_admin_vicky',
            email: SUPER_ADMIN_EMAIL,
            displayName: 'Vicky Bhelave',
            username: 'vickybhelave',
            photoURL: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80',
            avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80',
            about: 'Developer – CalcChat App',
            status: 'Developer & Super Admin',
            isSuperAdmin: true,
            isAdmin: true,
            isVerified: true,
            online: true,
        ...
        const friendshipId = [superAdminUid, targetUser.uid].sort().join('_');
        await setDoc(doc(db, 'friends', friendshipId), {
          user1Id: superAdminUid,
    ...
    const applyFallbackUser = (fbUser: FirebaseUser, requireUsername = true) => {
      ...
      const isSuperAdmin = fbEmail === SUPER_ADMIN_EMAIL;
    ...
    const ensureUserDocument = async (fbUser: FirebaseUser) => {
      ...
      if (!snap || !snap.exists()) {
        const localAvatar = localStorage.getItem(`calcchat_custom_avatar_${fbUser.uid}`);
        const photoURL = localAvatar || fbUser.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=1200&auto=format&fit=crop&q=95';
    ...
    const unsubscribe = onAuthStateChanged(firebaseAuth, async (fbUser) => {
      ...
      let localSavedUsername: string | null = null;
      ...
      try {
        ...
        if (snap && snap.exists()) {
          ...
          const fbEmail = (fbUser.email || '').toLowerCase();
          const isSuperAdmin = fbEmail === SUPER_ADMIN_EMAIL || Boolean(uData.isSuperAdmin);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(usersQuery, (snapshot) => {
      ...
      snapshot.docs.forEach(docSnap => {
        const data = docSnap.data();
        const resolvedAvatar = data.avatar || data.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80';
        uList.push({
          uid: docSnap.id,
          id: docSnap.id,
  ...
  useEffect(() => {
    ...
    const updateCombinedFriends = async () => {
      ...
      const myAvatar = user.avatar || (authUser ? localStorage.getItem(`calcchat_custom_avatar_${authUser.uid}`) : null) || authUser.photoURL || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80';
    ...
    const updateGroupContactsFromMaps = () => {
      const fetchedGroups: Contact[] = Array.from(docsGroupMap.entries()).map(([gId, data]) => {
        ...
        if (data.activeCall && data.activeCall.status === 'active') {
          ...
          const grpNotif = getContactNotificationSettings(data.groupId || gId);
          ...
          snapshot.docChanges().forEach((change) => {
            if (change.type === 'added') {
              ...
              if (isNewMessage && data.senderId && data.senderId !== authUser.uid) {
                ...
                const isNotifAllowed = isGroup
                  ? (globalGrpNotif && senderNotif.chatNotifications && targetNotif.chatNotifications)
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(statusQuery, (snapshot) => {
      ...
      snapshot.docs.forEach(d => {
        const data = d.data();
        const isMentioned = Array.isArray(data.mentionedUserIds) && data.mentionedUserIds.includes(authUser.uid);
        const isMentionedByUsername = user.username && Array.isArray(data.mentions) && data.mentions.some((m: string) => m.toLowerCase().replace('@', '') === user.username.toLowerCase());

        if (allowedUids.has(data.userId) || isMentioned || isMentionedByUsername) {
          ...
          if (nowMs - createdMs <= twentyFourHoursMs) {
            list.push({
              ...
              originalStatusId: data.originalStatusId || undefined,
              originalCreatorId: data.originalCreatorId || undefined,
              originalCreatorUsername: data.originalCreatorUsername || undefined,
              originalCreatorName: data.originalCreatorName || undefined,
              createdAt: data.createdAt,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsub = onSnapshot(notifQuery, (snapshot) => {
        ...
        // Find latest unread mention notification
        const latestUnreadMention = list.find(n => !n.read && n.type === 'status_mention');
        if (latestUnreadMention) {
          setActiveMentionNotification(latestUnreadMention);
        }
      }, (err) => {
  ...
  const openMentionedStatus = async (statusId: string) => {
    if (!statusId) return { success: false, message: 'Invalid status ID.' };

    // 1. Check if status exists in local statusUpdates
    let targetStatus = statusUpdates.find(s => s.id === statusId);

    // 2. Fetch directly from Firestore if not in local array
    if (!targetStatus && db) {
      try {
        ...
        if (snap.exists()) {
          ...
          const nowMs = Date.now();
          let createdMs = nowMs;
          if (data.createdAt?.toMillis) createdMs = data.createdAt.toMillis();
        ...
        console.warn('Error fetching mentioned status:', e);
      }
    }
    ...
    const notif = notifications.find(n => n.statusId === statusId);
    if (notif) {
  ...
  // Complete Username Setup (Step 2)
  const completeUsernameSetup = async (username: string, displayName: string) => {
    if (!authUser) return;

    const uLower = username.toLowerCase().trim();
    if (!uLower || uLower.length < 3 || uLower.length > 20) {
      throw new Error('Username must be 3–20 characters');
    }

    const validRegex = /^[a-zA-Z0-9_]+$/;
    if (!validRegex.test(uLower)) {
      throw new Error('Only letters, numbers, and underscores are allowed');
    }
    ...
    // Proceed to Step 3: Chat Password Setup
    setOnboardingStep('password');
    setNeedsUsername(true);
  };

  // Complete Chat Password Setup (Step 3)
  const completeChatPasswordSetup = async (passcode: string) => {
    if (!authUser) return;
    ...
    const userRef = doc(db, 'users', authUser.uid);
    const updatedSettings = { ...settings, passcode: cleanPasscode };
    ...
    await updateDoc(userRef, {
      ...
      console.warn('Failed to update passcode in user doc:', err);
    });
  ...
  const postStatusUpdate = async (
    ...
    try {
      const docRef = await addDoc(collection(db, 'status'), {
        ...
        originalStatusId: originalStatusId || null,
        originalCreatorId: originalCreatorId || null,
        originalCreatorUsername: originalCreatorUsername || null,
      ...
      if (Array.isArray(mentionedUserIds) && mentionedUserIds.length > 0) {
        ...
        const notifPromises = mentionedUserIds.map(async (targetUid) => {
          ...
          await addDoc(collection(db, 'notifications'), {
            ...
            body: `${ownerName} mentioned you in their Status.`,
            statusId: docRef.id,
            ownerId: currentAuthUser.uid,
            ownerName: ownerName,
            ownerPhoto: ownerAvatar,
            read: false,
            createdAt: serverTimestamp(),
  ...
  const sendMessage = async (
    ...
    const targetUser = allRegisteredUsers.find(u => u.uid === receiverId || u.id === receiverId) || contacts.find(c => c.id === receiverId);
    const isTargetSuperAdmin = receiverId === 'super_admin_vicky' || targetUser?.email?.toLowerCase() === SUPER_ADMIN_EMAIL || targetUser?.isSuperAdmin;

    // Super Admin bypasses friend and block restrictions when sending or receiving!
    if (!isSelfSuperAdmin && !isTargetSuperAdmin) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(incomingQuery, (snapshot) => {
      snapshot.docs.forEach((docSnap) => {
        ...
        const callerNotif = getContactNotificationSettings(data.callerId);
        const globalCallNotif = localStorage.getItem('calcchat_global_notify_calls') !== 'false';
  ...
  const toggleUserAdminStatus = async (targetUid: string, nextIsAdmin: boolean) => {
    if (!authUser) return;
    const isSelfSuperAdmin = authUser.email?.toLowerCase() === SUPER_ADMIN_EMAIL || user.isSuperAdmin;
    if (!isSelfSuperAdmin) {
  ...
  const toggleUserVerifiedBadge = async (targetUid: string, nextIsVerified: boolean) => {
    if (!authUser) return;
    const isSelfSuperAdmin = authUser.email?.toLowerCase() === SUPER_ADMIN_EMAIL || user.isSuperAdmin;
    if (!isSelfSuperAdmin) {
  ...
  const followUserDirectly = async (targetUid: string) => {
