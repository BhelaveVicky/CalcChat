  ...
  interface VaultContextType {
    ...
    sendMessage: (receiverId: string, text: string, media?: MediaAttachment, replyTo?: Message['replyTo'], statusReply?: StatusReplyData, statusReaction?: StatusReactionData, isForwarded?: boolean, mentions?: MentionData[]) => Promise<void>;
    editMessage: (contactId: string, msgId: string, newText: string, mentions?: MentionData[]) => Promise<void>;
  ...
  export const VaultProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Listener for Notifications (Mentions, etc.)
    useEffect(() => {
      if (!authUser || needsUsername) return;
      ...
      }, (err) => handleFirestoreError('Notifications snapshot error:', err, () => setAppNotifications([])));
    ...
    // Extract mentions from text helper
    const extractMentionsFromText = (
      txt: string,
      explicitMentions?: MentionData[]
    ): MentionData[] => {
      const map = new Map<string, MentionData>();
      if (explicitMentions && Array.isArray(explicitMentions)) {
        explicitMentions.forEach((m) => {
          if (m && m.username) {
            map.set(m.username.toLowerCase(), m);
          }
        });
      }
          ...
          if (!map.has(username)) {
            ...
            if (found) {
              map.set(username, {
                ...
                displayName: found.displayName || found.name || username,
              });
            }
          }
        });
    ...
    const sendMessage = async (
      ...
      const finalMentions = extractMentionsFromText(text || '', mentions);
      ...
      if (finalMentions && finalMentions.length > 0) {
        const senderUsername = user.username || 'user';
        const senderDisplayName = user.name || authUser.displayName || 'Someone';

        for (const m of finalMentions) {
          if (m.userId && m.userId !== authUser.uid) {
            addDoc(collection(db, 'notifications'), {
              userId: m.userId,
              type: 'mention',
              title: 'New Mention',
              body: `You were mentioned by @${senderUsername}`,
              senderId: authUser.uid,
              ...
              read: false,
              createdAt: serverTimestamp(),
            }).catch((err) => console.warn('Failed to save mention notification:', err));
          }
        }
      }
    ...
    const editMessage = async (contactId: string, msgId: string, newText: string, mentions?: MentionData[]) => {
      if (!authUser) return;
      ...
      const finalMentions = extractMentionsFromText(newText, mentions);
      ...
      if (finalMentions && finalMentions.length > 0) {
        const senderUsername = user.username || 'user';
        const senderDisplayName = user.name || authUser.displayName || 'Someone';
        const oldMentionUserIds = (msg?.mentions || []).map((m) => m.userId);

        for (const m of finalMentions) {
          if (m.userId && m.userId !== authUser.uid && !oldMentionUserIds.includes(m.userId)) {
            addDoc(collection(db, 'notifications'), {
              userId: m.userId,
              type: 'mention',
              title: 'New Mention',
    ...
    const toggleVideoCall = () => {
      ...
      const callId = activeCallRef.current?.id;
      if (callId && authUser) {
        const isCaller = activeCallRef.current?.direction === 'outgoing';
